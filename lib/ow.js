// I`am use code part from https://github.com/vkfont/owjs and https://github.com/benediktarnold/owfs libraries.
// Library for talk with owserver
// Usage:
// var ownet = require("./lib/ow");
// var owserver = new ownet.owserver('owserver.host',owserver.port);
    ...
    this.ls = function(path, callback){
        this.request(path, OW_DIRALL, 0, function(data){
            if(!data[0].error){
                ...
                data[0] = data[data.length - 1].error;
            }
            callback(data);
         });
